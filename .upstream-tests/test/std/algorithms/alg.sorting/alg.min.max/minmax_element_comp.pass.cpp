...
{
    typedef cuda::std::greater<int> Compare;
    Compare comp;
...
test(int N)
{
    ...
    for (int i = 0; i < N; ++i)
        a[i] = i;
    cuda::std::shuffle(a, a+N, randomness);
...
test()
{
    ...
    typedef cuda::std::greater<int> Compare;
    Compare comp;
    cuda::std::pair<Iter, Iter> p = cuda::std::minmax_element(Iter(a), Iter(a+N), comp);
...
{
#if TEST_STD_VER >= 14
    constexpr auto p = cuda::std::minmax_element(il, il+8, less());
