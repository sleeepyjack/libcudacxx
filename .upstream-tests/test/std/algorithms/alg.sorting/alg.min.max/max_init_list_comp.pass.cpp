...
#if TEST_STD_VER >= 14
    {
    static_assert(cuda::std::max({1, 3, 2}, cuda::std::greater<int>()) == 1, "");
    static_assert(cuda::std::max({2, 1, 3}, cuda::std::greater<int>()) == 1, "");
    static_assert(cuda::std::max({3, 2, 1}, cuda::std::greater<int>()) == 1, "");
    }
#endif
